...
public class InsertDeleteGetrandomO1DuplicatesAllowed {
    ...
    class RandomizedCollection {
        ...
        public boolean remove(int val) {
            ...
            Iterator<Integer> idxOfVals = idx.get(val).iterator();
            Integer inxOfVal = idxOfVals.next();

            Integer lastNum = nums.get(nums.size() - 1);
            nums.set(inxOfVal, lastNum);

            idx.get(val).remove(inxOfVal);
            idx.get(lastNum).remove(nums.size() - 1);

            if (inxOfVal < nums.size() - 1) {
                idx.get(lastNum).add(inxOfVal);
            }
